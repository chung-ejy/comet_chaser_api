    ...
    try:
        if request.method == "GET":
            ...
            if params == "trade_params":
                final = comet.retrieve("btc_trading_params").round(decimals=2)
                ...
                final = comet.retrieve(f"cloud_test_{params}").round(decimals=2)[["time","crypto","signal","velocity","inflection","p_sign_change","price","bid","ask"]]
                ...
                final = comet.retrieve(f"cloud_test_{params}").round(decimals=2)
                ...
                buys = comet.retrieve("cloud_test_completed_buys").round(decimals=2)
                sells = comet.retrieve("cloud_test_completed_sells").round(decimals=2)
                pending_buys = comet.retrieve("cloud_test_pending_buys").round(decimals=2)
                pending_sells = comet.retrieve("cloud_test_pending_sells").round(decimals=2)
                ...
                trades = comet.retrieve("cloud_test_completed_trades").round(decimals=2)
                pending_trades = comet.retrieve("cloud_test_pending_trades").round(decimals=2)
                ...
                complete = comet.retrieve(f"cloud_test_{params}").round(decimals=2).iloc[::-1].head(10).to_dict("records")
            comet.disconnect()
